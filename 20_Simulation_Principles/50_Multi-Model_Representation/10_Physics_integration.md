...
$$\frac{d\boldsymbol{p}}{dt}=f_{\text{vol}}+f_{\text{surf}}$$
$$\frac{D}{Dt}\int_{\Omega}\rho%20\boldsymbol{v}d\Omega%20=\int_{\Omega}%20\rho%20\boldsymbol{b}d\Omega%20+\int_{\Gamma}\boldsymbol{t}d\Gamma$$
$$\int_{\Omega}%20\rho%20\frac{D\boldsymbol{v}}{Dt}d\Omega%20=\int_{\Omega}%20\rho%20\boldsymbol{b}d\Omega%20+\int_{\Gamma}n\cdot%20\boldsymbol{\sigma}d\Gamma$$
$$\int_{\Omega}\rho%20\frac{D\boldsymbol{v}}{Dt}d\Omega%20=\int_{\Omega}\rho%20\boldsymbol{b}d\Omega%20+\int_{\Omega}\frac{\partial%20\sigma_{ij}}{\partial%20x{i}}d\Omega$$
...
$$\int_{\Omega}f(x)d\Omega%20=\sum_{e=0}^E%20\int_{V_e}f(x)dV_e$$
...
The shape functions $$\phi$$ can be expressed with regard to local coordinates $$(\xi%20,\eta%20,\zeta%20)$$, corresponding to a reference configuration of the element. Even if the elements get distorted during the simulation, each element can always be mapped back to one common reference configuration with local coordinates $$(\xi%20,\eta%20,\zeta%20)$$, as shown in the figure below. In other words, for any point in space $$\textbf{x}=(x,y,z)$$ inside the element $$K$$ a corresponding point $$\boldsymbol{\xi}=(\xi%20,\eta%20,\zeta%20)$$ can be found in the reference space of the element $$\hat{K}$$.
...
$$T^K:\hat{K}%20\longrightarrow%20K$$
$$\boldsymbol{\xi}=(\xi%20,\eta%20,\zeta%20)\longrightarrow%20\textbf{x}=T^K(\boldsymbol{\xi})=\sum_{i=0}^{N}x_i%20\phi_i(\boldsymbol{\xi})$$

This transformation $$T^K$$ is bijective if the determinant of the Jacobian of the transformation is non-null $$det(J)\neq0$$. Moreover, the transformation $$T^K$$ will allow for the integration of the weak form by change of variables: instead of integrating over $$d\Omega=dxdydz$$, the integration will be reported on the reference element $$d\xi%20d\eta%20d\zeta$$. 


#### Example
Let's consider an example. A density of force $$\boldsymbol{b}$$ that depends linearly on the coordinates $$\textbf{x}=(x,y,z)$$: $$\boldsymbol{b}=\alpha%20\textbf{x}$$. The integration of this term can be done as follows:



$$\int_{\Omega}%20\phi_j%20\rho%20\boldsymbol{b}d\Omega%20=\sum_{e=0}^E%20\rho%20\int_{V_e}%20\phi_j%20\alpha%20\textbf{x}dV_e$$
$$\int_{\Omega}%20\phi_j%20\rho%20\boldsymbol{b}d\Omega%20=\sum_{e=0}^E%20\rho%20\alpha%20\int_{V_e}%20\phi_j%20\sum_{i=0}^{N}%20\phi_i%20x_i%20dV_e$$
$$\int_{\Omega}%20\phi_j%20\rho%20\boldsymbol{b}d\Omega%20=\sum_{e=0}^E%20\rho%20\alpha%20\int_{V_e}%20|det(J)|%20\sum_{i=0}^{N}\phi_j(\boldsymbol{\xi})%20\phi_i(\boldsymbol{\xi})%20x_i%20d%20\boldsymbol{\xi}$$




For each finite element, the shape function $$\phi_i$$ is defined regarding the local coordinates $$\boldsymbol{\xi}$$. The expression of  $$\phi_j(\boldsymbol{\xi})%20\phi_i(\boldsymbol{\xi})$$ results from it. Finally, using a [Gauss quadrature](https://en.wikipedia.org/wiki/Gaussian_quadrature) (or Gauss point integration), it is possible to find a numerical solution to this integral over the reference element.

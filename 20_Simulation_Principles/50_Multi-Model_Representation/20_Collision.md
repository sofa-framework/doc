...
- a [broad phase](../../components/components/collision/detection/algorithm/broadphases/broadphase/)
- a [narrow phase](../../components/components/collision/detection/algorithm/narrowphases/narrowphase)

Several [broad phase](../../components/components/collision/detection/algorithm/broadphases/broadphase/) and [narrow phase](../../components/components/collision/detection/algorithm/narrowphases/narrowphase) methods are available in SOFA.
...
The first step of the pipeline is the so-called [broad-phase](../../components/components/collision/detection/algorithm/broadphases/broadphase/). It aims at quickly and efficiently removing objects pairs that are not in collision.
...
The [narrow phase](../../components/components/collision/detection/algorithm/narrowphases/narrowphase) of detection can rely on collision models to detect a contact. Note that different collision models are available to detect a contact:
...
All collision detection methods will rely on intersection methods during the broad and/or [narrow phase](../../components/components/collision/detection/algorithm/narrowphases/narrowphase) in order to assess if the models do collide. Given 2 collision elements, these intersection methods test if an intersection is possible.
...
As output, the collision detection (further to the [narrow phase](../../components/components/collision/detection/algorithm/narrowphases/narrowphase)) returns pairs of geometric primitives with the corresponding collision points. The collision information is saved in a vector of DetectionOutput. This data structure is a generic description of a contact point, used for most collision models except special cases such as GPU-based collisions.
...
The colliding models returned by the [narrow phase](../../components/components/collision/detection/algorithm/narrowphases/narrowphase) are finally given to the ContactManager, which creates contact interactions of various types based on customizable rules. You can specify which one you want to use in the DefaultContactManager. Response has been implemented based on:

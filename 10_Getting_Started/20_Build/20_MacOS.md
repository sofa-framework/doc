...
⚠️ Under macOS, using Python from runSofa with SofaPython3 would cause a segmentation fault due to symbol duplication (the Python package from Conda Forge statically linked with libpython, conflicting with runSofa’s own linking). A patch (proposed in PR [#394](https://github.com/sofa-framework/SofaPython3/pull/394)) is currently used in our Conda packages.
